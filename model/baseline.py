...
unit_gcn
self.down: 残差连接
num_sebset: 3种samples，自身，近心，离心
self.PA可以理解为一种骨架的一种common sense的pattern
self.A是骨架的先验
A1是学出来的data-driven的骨架信息
inter_channels = out_channels // coff_embedding：embedding映射，通道降采样
conv_d：对AX的embedding
'''
'''
class unit_gcn_1(nn.Module):
    def __init__(self, in_channels, out_channels, A, G_part, G_body, num_point, coff_embedding=4, num_subset=3):
    ...
    def forward(self, x):
        ...
        y = None
        for i in range(self.num_subset): #3种samples，自身，近心，离心。为每一种sample学习一个data-dependent的unique graph
            A1 = self.conv_a[i](x).permute(0, 3, 1, 2).contiguous().view(N, V, self.inter_c * T)
            A2 = self.conv_b[i](x).view(N, self.inter_c * T, V)
            A1 = self.soft(torch.matmul(A1, A2) / A1.size(-1)) #C
            A1 = A1 + A[i]
            A2 = x.view(N, C * T, V)
            z = self.conv_d[i](torch.matmul(A2, A1).view(N, C, T, V)) #embed(AX)
            y = z + y if y is not None else z
...
class SDE(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=25,
    ...
    def forward(self, x):
        ...
        stacked_similarity = self.bn_similarity(stacked_similarity).view(N * 1, 3, self.groups, V, V).sum(dim=1)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, G_part, G_body, num_point, kernel_size=5, stride=1, dilations=[1,2], residual=True):
        ...
        self.spa_attn = SDE(out_channels,out_channels) # data_driven v2v relations
        ...
        elif (in_channels == out_channels) and (stride == 1): #输入通道=输出通道且stride=1表示不做降采样，residual不会变化，所以residual=x
            self.residual = lambda x: x
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3):
        ...
        if graph is None:
            ...
            Hypergraph = import_class('graph.ntu_rgb_d.Hypergraph')
