...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if arg.cal_params:
            ...
            for i in range(0, 1000):
                img = torch.randn(1, 3, 64, 25, 2)  # data_length=4
                img = img.cuda()
                #img = self.data_loader['train']
            ...
            print('FPS = {:.1f}'.format(32 / np.mean(run_time)))

            flops, params = profile(self.model, inputs=(img,), verbose=False)
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                for batch_idx, (data, label, index) in enumerate(process):
                    ...
                    #todo jsut for test
                    #output, temp= self.model(data)

                    #weight = self.model.module.fc.weight.squeeze().detach().cpu().numpy()
                    #feature = temp.detach().cpu().numpy()
                    #self.result = np.einsum('kc,bctvm->bktvm', weight, feature)
                    # g_a = self.model.module.l6.gcn1.A.data.sum(0).cpu().numpy()
                    # g_aa = self.model.module.l6.gcn1.PA.data.sum(0).cpu().numpy()
                    # g_p = self.model.module.l6.gcn1.G_part.data.cpu().numpy()
                    # g_b = self.model.module.l6.gcn1.G_body.data.cpu().numpy()
                    # import matplotlib.pyplot as plt
                    # f, ax = plt.subplots(1, 3)
                    # #ax[0].imshow(A_binary_with_I, cmap='gray')
                    # ax[0].imshow(g_a+g_aa, cmap='gray')
                    # ax[1].imshow(g_p, cmap='gray')
                    # ax[2].imshow(g_b, cmap='gray')
                    #
                    # plt.savefig('/home/zys/code/MS-G3D/vis/' + 'vis' + '.eps' ,dpi=600,format='eps')
                    # plt.show()
    ...
    def show_important_joints(self):
        import logging
        self.result = self.result[26][26]
        first_sum = np.sum(self.result[:,:,0], axis=1)
