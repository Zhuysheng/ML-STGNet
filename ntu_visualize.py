...
def visualize(args):
    data_path = args.datapath or './data/{}/train_data_joint.npy'.format(args.dataset)
    label_path = args.labelpath or './data/{}/train_label.pkl'.format(args.dataset)
    ...
    def animate(skeleton):
        ...
        lab = labels[0][index]
        location = np.zeros((2, 64, 25, 2))
        lab_path = os.path.join('/home/data/skeleton/nturgbd_raw/nturgb+d_skeletons', lab)
        ...
        connecting_joint = np.array(
            [2, 1, 21, 3, 21, 5, 6, 7, 21, 9, 10, 11, 1, 13, 14, 15, 1, 17, 18, 19, 2, 23, 8, 25, 12])

        pause, frames = 0.1, range(location.shape[1])
        plt.figure()
        plt.ion()
        for t in frames:
            if np.sum(location[:, t, :, :]) == 0:
                break
            ...
            plt.pause(pause)
            img = plt.imread('/home/zhuyisheng/code/MS-G3D/image-001.jpg')
            fig = plt.subplots()
            ax.imshow(img)
        plt.ioff()
        plt.show()
        ...
        plt.savefig('/home/zhuyisheng/code/MS-G3D/vis/' + str(skeleton_index[0]) + '.jpg')
